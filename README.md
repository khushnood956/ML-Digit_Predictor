...
curl -X POST "http://localhost:8080/api/predict" \
     -H "Content-Type: application/json" \
     -d '{"image_path": "sample_image.jpg"}'
```

## Troubleshooting

1. **Port conflicts**: Ensure ports 8000 and 8080 are available
2. **Model not found**: Make sure `cnn_model.h5` exists in the root directory
3. **CORS issues**: The services are configured to handle cross-origin requests
4. **Memory issues**: Increase JVM heap size if needed: `-Xmx2g`

## Future Enhancements

- Add model monitoring and metrics
...
- **Gateway Service**: Spring Boot (Java)
